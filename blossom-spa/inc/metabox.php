<?php 
/**
* Blossom Spa Metabox for Sidebar Layout
*
* @package Blossom_Spa
*
*/ 

function blossom_spa_add_sidebar_layout_box(){
    ...
    */
    if( ( ! in_array( $template, $templates ) && $shop_id != $post_id ) && ( $page_for_posts != $post_id ) ){
        add_meta_box( 
            'blossom_spa_sidebar_layout',
            __( 'Sidebar Layout', 'blossom-spa' ),
            'blossom_spa_sidebar_layout_callback', 
            'page',
            'normal',
            'high'
        );
...
function blossom_spa_sidebar_layout_callback(){
    ...
    <table class="form-table">
        <tr>
            <td colspan="4"><em class="f13"><?php esc_html_e( 'Choose Sidebar Template', 'blossom-spa' ); ?></em></td>
        </tr>
        <tr>
            <td>
                <?php  
                    foreach( $blossom_spa_sidebar_layout as $field ){  
                        $layout = get_post_meta( $post->ID, '_blossom_spa_sidebar_layout', true ); ?>
                        <div class="hide-radio radio-image-wrapper" style="float:left; margin-right:30px;">
                            <input id="<?php echo esc_attr( $field['value'] ); ?>" type="radio" name="blossom_spa_sidebar_layout" value="<?php echo esc_attr( $field['value'] ); ?>" <?php checked( $field['value'], $layout ); if( empty( $layout ) ){ checked( $field['value'], 'default-sidebar' ); }?>/>
...
function blossom_spa_save_sidebar_layout( $post_id ){
    ...
    $layout = isset( $_POST['blossom_spa_sidebar_layout'] ) ? sanitize_key( $_POST['blossom_spa_sidebar_layout'] ) : 'default-sidebar';

    if ( array_key_exists( $layout, $blossom_spa_sidebar_layout ) ) {
        update_post_meta( $post_id, '_blossom_spa_sidebar_layout', $layout );
    } else {
        delete_post_meta( $post_id, '_blossom_spa_sidebar_layout' );
    }        
}
